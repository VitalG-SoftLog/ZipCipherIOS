...
typedef struct
{
    unsigned char *read_buffer;                  /* internal buffer for compressed data */
        ...
        while((err!=UNZ_ERRNO) && (acc < file_info.size_file_extra))
        {
            ...
            else if (headerid == 0x9901)
            {
                ...
                if (unz64local_getShort(&s->z_filefunc, s->filestream_with_CD,&uL) != UNZ_OK)
                    err=UNZ_ERRNO;
                if ((uL != 1) && (uL != 2)) // FIX: Add version 2
                    err = UNZ_ERRNO;
...
extern int ZEXPORT unzOpenCurrentFile3(unzFile file, int* method, int* level, int raw, const char* password)
{
    ...
    pfile_in_zip_read_info->read_buffer=(unsigned char*)ALLOC(UNZ_BUFSIZE);
    ...
    if (password != NULL)
    {
        ...
        if (s->cur_file_info.compression_method == AES_METHOD)
        {
            ...
            fcrypt_init(s->cur_file_info_internal.aes_encryption_mode, (unsigned char*)password, strlen(password), saltvalue,
